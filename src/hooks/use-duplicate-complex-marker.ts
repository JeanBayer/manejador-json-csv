...
export const useDuplicateComplexMarker = () => {
  const { text, setText, jsonOutput } = useInputWithFormat(
    `${RoutePath.DUPLICATE_COMPLEX_MARKER}-input-text`
  );
  const [matchField, setMatchField, matchFieldDebounce] = useStateDebounce(
    `${RoutePath.DUPLICATE_COMPLEX_MARKER}-match-field`,
    "identificador"
  );
  const [outputFormat, setOutputFormat, outputFormatDebounce] =
    useStateDebounce(
      `${RoutePath.DUPLICATE_COMPLEX_MARKER}-output-format`,
      "json"
    );
